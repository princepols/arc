/**
 * Arc AI - Sidebar (Mobile-Responsive)
 */

import { useState, useEffect, useRef } from 'react'
import { sessionsAPI } from '../utils/api'
import { Plus, Search, Trash2, LogOut, MessageSquare, X, Pencil, Check } from 'lucide-react'
...
function SessionItem({ session, active, onSelect, onDelete, onRename }) {
  ...
  const submitRename = async () => {
    ...
    if (val && val !== session.title) await onRename(session.id, val)
    else setRenameVal(session.title || 'New Chat')
  ...
  return (
    ...
    >
      ...
      <div style={s.itemBody}>
        {renaming ? (
          <input
            ref={inputRef} style={s.renameInput} value={renameVal}
      ...
      {!renaming && (hovered || menuOpen || active) && (
        <div style={s.actions} onClick={e => e.stopPropagation()} ref={menuRef}>
          <button style={s.iconBtn('var(--text-muted)')} onClick={() => { setRenaming(true); setMenuOpen(false) }} title="Rename">
            <Pencil size={11} />
          </button>
          <button style={s.iconBtn(confirmDel ? '#f87171' : 'var(--text-muted)', confirmDel)} onClick={handleDelete} title={confirmDel ? 'Click again to confirm' : 'Delete'}>
      ...
      {renaming && (
        <button style={{ ...s.iconBtn('#4ade80'), marginTop: 6 }} onClick={e => { e.stopPropagation(); submitRename() }} title="Save">
          <Check size={11} />
        </button>
      )}
    </div>
...
export default function Sidebar({ user, profile, activeSession, onSelectSession, onNewChat, onLogout, refreshTrigger, onSessionRenamed, onOpenProfile, className = '' }) {
  ...
  return (
    <aside style={s.sidebar} className={`arc-sidebar ${className}`}>
      ...
      <div style={s.searchWrap}>
        ...
        <input style={s.searchInput} placeholder="Search chats…" value={search} onChange={e => setSearch(e.target.value)} />
      ...
      <div style={s.list}>
        ...
        {displayed.map(session => (
          <SessionItem
            key={session.id} session={session} active={activeSession === session.id}
            onSelect={onSelectSession} onDelete={handleDelete} onRename={handleRename}
          />
        ))}
      </div>
      ...
      <div style={s.footer}>
        <div
          ...
          title="Edit profile"
        >
          <div style={s.avatar}>{(profile?.displayName || user.username)[0].toUpperCase()}</div>
          <span style={s.username}>{profile?.displayName || user.username}</span>
        </div>
...
const s = {
  ...
  clearBtn: { background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', display: 'flex', padding: 0 },
  ...
  itemTitle: { fontSize: 12, fontWeight: 500, color: 'var(--text-primary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' },
  ...
  iconBtn: (color, highlight) => ({
    ...
    justifyContent: 'center', transition: 'all 0.13s', padding: 0, flexShrink: 0,
  }),
  footer: {
    display: 'flex', alignItems: 'center', justifyContent: 'space-between',
